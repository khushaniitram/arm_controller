...
export default function ControlPanel({ onJogJoint, onJogCartesian, onStop, speed, onSpeedChange, lockNeedle = false }: ControlPanelProps) {
  ...
  const startPointerJog = (event: React.PointerEvent<HTMLButtonElement>, startJog: () => void) => {
    event.preventDefault();
    event.currentTarget.setPointerCapture(event.pointerId);
    startJog();
  };

  const stopPointerJog = (event: React.PointerEvent<HTMLButtonElement>) => {
            ...
            {joints.map(j => {
              ...
              return (
                <div key={j} className="flex gap-1">
                  <button 
                    onPointerDown={(event) => startPointerJog(event, () => onJogJoint(j, "-"))}
                    onPointerUp={stopPointerJog}
                    onPointerCancel={stopPointerJog}
          ...
          <div className="grid grid-cols-1 gap-2">
            {cartesian.map(axis => (
              <div key={axis} className="flex gap-2">
                <button 
                  onPointerDown={(event) => startPointerJog(event, () => onJogCartesian(axis, "-"))}
                  onPointerUp={stopPointerJog}
                  onPointerCancel={stopPointerJog}
                  onLostPointerCapture={onStop}
